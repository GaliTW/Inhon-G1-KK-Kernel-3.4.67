...
static int mpu6050_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  if ((err = mpu6050_create_attr(&mpu6050_driver.driver)))
  {
  ...
  err = batch_register_support_info(ID_ACCELEROMETER,obj->hw->is_batch_supported);
  if(err)
  {
...
static int mpu6050_i2c_remove(struct i2c_client *client)
{
  int err = 0;  
    ...
    if ((err =  mpu6050_delete_attr(&mpu6050_driver.driver)))
  {
...
static int __init mpu6050gse_init(void)
{
  ...
  if ( platform_driver_register( &mpu6050_driver ) )
  {
...
{
  GSE_FUN();
  platform_driver_unregister( &mpu6050_driver ) ;
}
/*----------------------------------------------------------------------------*/
